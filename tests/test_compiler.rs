...
fn test_cost_clause_compiles() {
    let compiler = PolicyCompiler::new();
    let policy = "All actions must be logged by SYSTEM. Cost of logging cannot exceed 1000 USD per month.";
    let result = compiler.compile(policy);
...
fn test_cost_with_tokens_unit() {
    let compiler = PolicyCompiler::new();
    let policy = "Resource usage must be tracked by SYSTEM. Quota of 5000 tokens must be enforced by SERVICE.";
    let result = compiler.compile(policy);
...
fn test_cost_without_subject_fails() {
    ...
    let policy = "Cost must be for the SYSTEM.";
    let result = compiler.compile(policy);
...
fn test_cost_mention_without_explicit_unit_fails() {
    let compiler = PolicyCompiler::new();
    let policy = "All actions must be logged by SYSTEM. Cost of logging cannot exceed $1000 per month.";
    let result = compiler.compile(policy);
